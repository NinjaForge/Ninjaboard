...
//class helper
class modNinjaBoardPostsHelper
{
...
/*        $show_public = $params->get('show_public', 1);
		...
		//get menu id
        $itemId = modNinjaBoardPostsHelper::getItemId();
		//get com menu id
		$comItemid = modNinjaBoardPostsHelper::getComItemid($name_link);
...
/*        $i = $show_public + 2 * $show_regualar + 4 * $show_special;
		...
		if (($which_posts == 6) || ($which_posts == 7) || ($which_posts == 8)){
			$sql_forum = '';
		if($url_option=="com_ninjaboard"&&$url_view=="forum"){$current_forums = $url_id;}
		if($url_option=="com_ninjaboard"&&$url_view=="topic"){$current_forums = (modNinjaBoardRelatedHelper::getForumId($url_id));}
			if ( !empty($current_forums) && empty($only_forums) ){$include_forums = $current_forums;}
			elseif ( empty($current_forums) && !empty($only_forums) ){$include_forums = $only_forums;}
			elseif ( !empty($current_forums) && !empty($only_forums) ){$include_forums = array_merge((array)$current_forums,(array)$only_forums);}
		}else{
			$include_forums = $only_forums;
		}
        $sql_forum = '';
        if ($include_forums)
        {
            $forums =$include_forums;
            ...
            $include_forums = implode(',', $forums);
            if ($include_forums)
            {
                if ($forums)
                {
                    $sql_forum = " AND (f.ninjaboard_forum_id IN (" . $include_forums .
                        ") OR f.parent_id IN (" . $include_forums . "))";
                }
                else
                {
                    $sql_forum = " AND f.ninjaboard_forum_id IN (" . $include_forums . ")";
        ...
        //order by posts
        if (($which_posts == 0) || ($which_posts == 1) || ($which_posts == 2))
        {
            $sql_field_order = "ORDER BY p.created_time ";
        } elseif (($which_posts == 3) || ($which_posts == 4) || ($which_posts == 5))
        {
            $sql_field_order = "ORDER BY t.hits ";
        } elseif (($which_posts == 6) || ($which_posts == 7) || ($which_posts == 8))
        {
            $sql_field_order = "GROUP BY p.subject ORDER BY p.created_time ";
        } elseif (($which_posts == 9) || ($which_posts == 10) || ($which_posts == 11))
        {
			$sql_field_order = "ORDER BY RAND()";
		}
        //ASC or DESC
        if ($which_posts >= 9)
        {
            $sql_order_by = "";
        } elseif (($order_by == 0) && ($which_posts <= 8))
        {
            $sql_order_by = "DESC";
        } elseif (($order_by == 1) && ($which_posts <= 8))
        {
            $sql_order_by = "ASC";
        }
        //post type
        if (($which_posts = 0) || ($which_posts = 3) || ($which_posts = 6) || ($which_posts = 9))
        {
            $post_type = "Left Join #__ninjaboard_topics AS t ON p.ninjaboard_topic_id = t.ninjaboard_topic_id";
        } elseif (($which_posts = 1) || ($which_posts = 4) || ($which_posts = 7) || ($which_posts = 10))
        {
            $post_type = "Left Join #__ninjaboard_topics AS t ON p.ninjaboard_post_id = t.first_post_id";
        } elseif (($which_posts = 2) || ($which_posts = 5) || ($which_posts = 8) || ($which_posts = 11))
        {
        ...
        foreach ($rows as $row)
        {
			...
			$row->message = modNinjaBoardPostsHelper::parseBBcode($row->message);
			...
			$item->online_status = (modNinjaBoardPostsHelper::getOnlineStatus($row->userid)) ? '<span id="nb-posts-online">' . JText::_('ONLINE') . '</span>' : '<span id="nb-posts-offline">' . JText::_('OFFLINE') . '</span>';
			$s = $item_format;
