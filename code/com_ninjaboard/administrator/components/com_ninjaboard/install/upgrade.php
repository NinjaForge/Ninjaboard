 ...
 */
 ...
 /**
 * @TODO Upgrade class, will be optimized and OOP
 */
 class ComNinjaboardInstallUpgrade
 {
 	...
 	public function query()
 	{
		...
		if(!isset($tables['topics']['sticky_on']))
		{
		    ...
		    $db->query();
		    $db->setQuery("ALTER TABLE `#__ninjaboard_topics` ADD INDEX `sticky` (`sticky`);");
		    $db->query();

		    $db->setQuery("ALTER TABLE `#__ninjaboard_topics` ADD `sticky_on` datetime NOT NULL DEFAULT '0000-00-00 00:00:00'  AFTER `sticky`;");
			$db->query();
			$db->setQuery("ALTER TABLE `#__ninjaboard_topics` ADD INDEX `sticky_on` (`sticky_on`);");
		    $db->query();

		    $db->setQuery("ALTER TABLE `#__ninjaboard_topics` ADD `sticky_by` int(11) unsigned NOT NULL default 0  AFTER `sticky_on`;");
			$db->query();
			$db->setQuery("ALTER TABLE `#__ninjaboard_topics` ADD INDEX `sticky_by` (`sticky_by`);");
		    $db->query();


		    $db->setQuery("ALTER TABLE `#__ninjaboard_topics` CHANGE `locked` `locked` TINYINT(1)  NOT NULL  DEFAULT '0';");
		    $db->query();
		    $db->setQuery("ALTER TABLE `#__ninjaboard_topics` ADD INDEX `locked` (`locked`);");
		    $db->query();

		    $db->setQuery("ALTER TABLE `#__ninjaboard_topics` ADD `locked_on` datetime NOT NULL DEFAULT '0000-00-00 00:00:00'  AFTER `locked`;");
			$db->query();
			$db->setQuery("ALTER TABLE `#__ninjaboard_topics` ADD INDEX `locked_on` (`locked_on`);");
		    $db->query();

		    $db->setQuery("ALTER TABLE `#__ninjaboard_topics` ADD `locked_by` int(11) unsigned NOT NULL default 0  AFTER `locked_on`;");
			$db->query();
			$db->setQuery("ALTER TABLE `#__ninjaboard_topics` ADD INDEX `locked_by` (`locked_by`);");
		    $db->query();


		    $db->setQuery("ALTER TABLE `#__ninjaboard_topics` CHANGE `resolved` `resolved` TINYINT(1)  NOT NULL  DEFAULT '0';");
		    $db->query();
		}
 	}
 }

$upgrader = new ComNinjaboardInstallUpgrade;
$upgrader->query();