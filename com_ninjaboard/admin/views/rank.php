	...
	//add ie6 css if needed
		...
		<form action="index.php?option=com_ninjaboard" method="post" name="adminForm">
			...
			<thead>
				<tr>
					...
					<th nowrap="nowrap" width="30%">
						<?php echo JHTML::_('grid.sort', 'NB_NAME', 'r.name', @$lists['filter_order_Dir'], @$lists['filter_order'], 'ninjaboard_rank_view'); ?>
					</th>
					<th nowrap="nowrap" width="60%">
						<?php echo JHTML::_('grid.sort', 'NB_DESCRIPTION', 'r.description', @$lists['filter_order_Dir'], @$lists['filter_order'], 'ninjaboard_rank_view'); ?>
					</th>
					<th nowrap="nowrap" width="5%">
						<?php echo JHTML::_('grid.sort', 'NB_MINPOSTS', 'r.min_posts', @$lists['filter_order_Dir'], @$lists['filter_order'], 'ninjaboard_rank_view'); ?>
					</th>									
					<th nowrap="nowrap" width="1%">
						<?php echo JHTML::_('grid.sort', 'NB_PUBLISHED', 'r.published', @$lists['filter_order_Dir'], @$lists['filter_order'], 'ninjaboard_rank_view'); ?>
					</th>																													
					<th nowrap="nowrap" width="1%">
						<?php echo JHTML::_('grid.sort', 'NB_ID', 'r.id', @$lists['filter_order_Dir'], @$lists['filter_order'], 'ninjaboard_rank_view'); ?>
					</th>			
				</tr>
			</thead>
			<tfoot>
			...
			for ($i=0, $n=count($rows); $i < $n; $i++) {
				...
				$task_published = $row->published ? 'ninjaboard_rank_unpublish' : 'ninjaboard_rank_publish';
				$alt_published = $row->published ? JText::_('NB_PUBLISHED') :  JText::_('NB_UNPUBLISHED');
								
				$link = 'index.php?option=com_ninjaboard&task=ninjaboard_rank_edit&hidemainmenu=1&cid[]='. $row->id;																																		
				?>
			...
			<input type="hidden" name="task" value="ninjaboard_rank_view" />
	...
	//add ie6 css if needed
		...
		<script language="javascript" type="text/javascript">
			function submitbutton(pressbutton) {
				...
				if (pressbutton == 'ninjaboard_rank_cancel') {
				...
				if (trim(form.name.value) == "") {
					alert("<?php echo JText::sprintf('NB_MSGFIELDREQUIRED', JText::_('NB_NAME'), JText::_('NB_RANK')); ?>");
				} else {
					submitform(pressbutton);
				}
			}
		...
		<form action="index.php" method="post" name="adminForm">
			<div class="col width-50">
				<fieldset class="adminform">
					<legend>
						<?php echo JText::_('NB_RANKDETAILS'); ?>
					</legend>
					<table class="admintable" cellspacing="1">
						...
						<tr>
							<td class="key">
								<label for="rank_file">
									<?php echo JText::_('NB_RANKIMAGEFILE'); ?>
								</label>
							</td>
							<td>
