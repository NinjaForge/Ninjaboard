...
/**
 * Ninjaboard Profile Field Set View
 *
 * @package Ninjaboard
 */
	...
	//add ie6 css if needed
		...
		<form action="index.php?option=com_ninjaboard" method="post" name="adminForm">
			...
			<thead>
				<tr>
					...
					<th nowrap="nowrap" width="90%">
						<?php echo JHTML::_('grid.sort', 'NB_NAME', 'p.name', @$lists['filter_order_Dir'], @$lists['filter_order'], 'ninjaboard_profilefieldset_view'); ?>
					</th>
					<th nowrap="nowrap" width="1%">
						<?php echo JHTML::_('grid.sort', 'NB_PUBLISHED', 'p.published', @$lists['filter_order_Dir'], @$lists['filter_order'], 'ninjaboard_profilefieldset_view'); ?>
					</th>										
					<th nowrap="nowrap" width="8%">
						<?php echo JHTML::_('grid.sort', 'NB_ORDER', 'p.ordering', @$lists['filter_order_Dir'], @$lists['filter_order'], 'ninjaboard_profilefieldset_view'); ?>
						<?php echo JHTML::_('grid.order', $rows, 'filesave.png', 'ninjaboard_profilefieldset_saveorder'); ?>
					</th>																													
					<th nowrap="nowrap" width="1%">
						<?php echo JHTML::_('grid.sort', 'NB_ID', 'p.id', @$lists['filter_order_Dir'], @$lists['filter_order'], 'ninjaboard_profilefieldset_view'); ?>
					</th>			
				</tr>
			</thead>
			<tfoot>
			...
			for ( $i=0, $n=count( $rows ); $i < $n; $i++ ) {
				...
				$task_published = $row->published ? 'ninjaboard_profilefieldset_unpublish' : 'ninjaboard_profilefieldset_publish';
				$alt_published = $row->published ? JText::_('NB_PUBLISHED') :  JText::_('NB_UNPUBLISHED');
								
				$link = 'index.php?option=com_ninjaboard&task=ninjaboard_profilefieldset_edit&hidemainmenu=1&cid[]='. $row->id;																																		
				?>
				<tr class="<?php echo "row$k"; ?>">
					...
					<td class="order">
						<span><?php echo $pageNav->orderUpIcon( $i, true, 'ninjaboard_profilefieldset_orderup', 'NB_ORDERUP', true ); ?></span>
						<span><?php echo $pageNav->orderDownIcon( $i, $n, true, 'ninjaboard_profilefieldset_orderdown', 'NB_ORDERDOWN', true ); ?></span>
			...
			<input type="hidden" name="task" value="ninjaboard_profilefieldset_view" />
		...
		<form action="index.php" method="post" name="adminForm" id="adminForm">
			<div class="col width-50">
				<fieldset class="adminform">
					<legend>
						<?php echo JText::_('NB_FIELDSETDETAILS'); ?>
					</legend>
