...
defined( '_JEXEC' ) or die( 'Restricted access' );
	...
	JToolBarHelper::title(JText::_('NB_ICONSETMANAGER'), 'ninjahdr');
	...
	JToolBarHelper::deleteList(JText::_('Are you sure you want to delete these instances').'?');
...
?>
		...
		<div id="submenu-box2">
			...
			<ul id="submenu2">
				...
				<li>
					<a class="active" href="index.php?option=com_ninjaboard&task=display&controller=iconset"><?php echo JText::_('NB_ICONSETS'); ?></a>
				</li>													
			</ul>
			<div class="clr"></div>
			</div>
		...
		<form id="Uploadfiles" name="Uploadfiles" action="index.php" enctype="multipart/form-data" method="post">
		  ...
		  <input type="hidden" name="task" value="uploadiconset" />
		...
		<form action="index.php?option=com_ninjaboard" method="post" name="adminForm">
			...
			for ($i=0, $n=count( $this->rows ); $i < $n; $i++) {
				...
				$img_default = $row->default_icon_set ? NB_ADMINIMAGES_LIVE.DL.'menu'.DL.'icon-16-default.png' : NB_ADMINIMAGES_LIVE.DL.'menu'.DL.'spacer.png';
				$alt_default = $row->default_icon_set ? JText::_('NB_DEFAULT') :  JText::_('NB_NOTDEFAULT');
		
				$link = 'index.php?option=com_ninjaboard&task=edit&controller=iconset&cid[]='. $row->file_name .'&hidemainmenu=1';																																		
				?>
