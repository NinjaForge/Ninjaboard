...
<?php 	if ($this->enableGuestName || $this->enableReplySubject) : 
//only show this fieldset if we have either a subject or a guest name to show. ?>
			<fieldset>
<?php		if ($this->enableGuestName) : ?>
...
<?php 		endif;
			if ($this->enableReplySubject) : ?>
				<div class="nbLeft">
					<label for="subject"><?php echo JText::_('NB_SUBJECT'); ?></label>
				</div>
				<div class="nbRight">
					<input id="subject" class="nbInputBox required" type="text" name="subject" size="60" maxlength="255" value="<?php echo $this->post->subject? $this->post->subject : JText::sprintf('NB_SUBJECT_RE',$this->topicSubject) ; ?>" />
				</div>
<?php 		endif; ?>
...
<?php 	endif; 
		if ($this->enableReplySubject) : 
//If we have disabled subjects, store a default one anyway, just in case we need to split a topic it isn't blank. This also saves us having to rework the tables and defaults ?>
					<input type="hidden" name="subject" value="<?php echo $this->post->subject? $this->post->subject : JText::sprintf('NB_SUBJECT_RE',$this->topicSubject) ; ?>" />
<?php 	endif; ?>
			...
			<fieldset>
				...
				<div class="nbRight">
					<textarea id="text" class="nbEditor required" rows="15" cols="70" name="text"><?php echo $this->post->text; ?></textarea>
				</div>
				...
				<div class="nbRight nbmt" id="nbSubmitButtons">
					...
					<button type="button" class="nb-buttons btnPreview" onclick="submitbutton('ninjaboardpreviewpost');"><span><?php echo JText::_('NB_PREVIEW'); ?></span></button>
		...
		<input type="hidden" name="task" value="ninjaboardsavepost" />
